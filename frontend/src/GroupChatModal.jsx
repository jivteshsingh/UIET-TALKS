...
import { Box, Spinner, useToast, useDisclosure, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, Button, FormControl, FormLabel, Input } from '@chakra-ui/react'
import { ChatState } from "./ChatProvider";
import axios from "axios";
import UserList from "./UserList";
import UserBadge from "./UserBadge";

function GroupChatModal({children,chats,setChats}) {
    ...
    <>
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <Box w="100%" d="flex" flexWrap="wrap">
          {selectedUsers.map((u) => (
            <UserBadge id={user._id} user={u} handleFunction={() => handleDelete(u) } />
          ))}
        </Box>

          {loading ? (<Spinner ml="auto" d="flex" />) : (
            searchResult?.slice(0, 4).map((user) => (
              <UserList id={user._id} user={user} handleFunction={() => handleGroup(user)} />
            ))
          )}
